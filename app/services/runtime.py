...
# ----------------------------------------------------------------------
# Перенос подарков между площадками
# ----------------------------------------------------------------------
#: Разрешён ли перенос подарков. Выключено по умолчанию и отдельно от
#: торговли: покупку можно отменить возвратом, а переданный подарок
#: не возвращается ничем. Это единственная необратимая операция бота.
KEY_TRANSFER = "TRANSFER_ENABLED"


def transfer_enabled() -> bool:
    """Разрешена ли передача подарков другому получателю."""
    return _as_bool(store.get(KEY_TRANSFER), False)
