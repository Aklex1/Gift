...
FIELDS: tuple[SecretField, ...] = (
    ...
    SecretField(
        ...
        hint="Официальный канал Portals (@portals_community) называет "
             "получателем @GiftsToPortals и требует отправлять подарки "
             "строго с личного аккаунта, не из канала. Там же "
             "предупреждают о подделках под этот бот, поэтому значение "
             "вписывается вручную и проверяется командой "
             "gift-cli transfer-target. Отправленный подарок не "
             "возвращается. Это НЕ адрес кошелька: тот принимает только "
             "GRAM и TON.",
        secret=False,
