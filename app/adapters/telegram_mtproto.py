...
#: Код валюты из ответа Telegram -> наша валюта.
#:
#: Telegram показывает цены подарков в валюте аккаунта: у одного это
#: звёзды, у другого рубли. Раньше всё, кроме звёзд, считалось GRAM —
#: и рублёвый floor 468 ₽ превращался в 468 GRAM, то есть в 46 700
#: звёзд вместо 399. Отсюда и брались ROI в десятки тысяч процентов.
CURRENCY_BY_CODE = {
    "XTR": Currency.STARS,
    "TON": Currency.TON,
    "USD": Currency.USD,
...
#: Во сколько раз значение больше самой валюты.
#:
#: Telegram отдаёт суммы «в наименьших единицах валюты». Для звезды
#: это сама звезда. Для фиата — сотая доля, и на рублях это проверено:
#: floor приходит как 46 800 при настоящих 468 ₽, а initial_sale_price
#: 23 200 при 176 звёздах даёт 1.32 ₽ за звезду — ровно её цену.
#:
#: Для GRAM масштаб не проверен: ни один живой ответ в нём ещё не
#: приходил. Взята наименьшая единица самой сети — нанотон. Если
#: окажется иначе, лот отсеется как расхождение источников, а не
#: превратится в находку века.
DIVISOR_BY_CODE = {
    "XTR": Decimal(1),
    "TON": Decimal(10) ** 9,
}
...
class TelegramAdapter(MarketAdapter):
    ...
    async def value_info(self, slug: str) -> dict[str, Any]:
        ...
        money = CURRENCY_BY_CODE.get(currency)
        divisor = DIVISOR_BY_CODE.get(currency, Decimal(100))
